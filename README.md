## Features

- [x] It should be able to register a user
- [x] It should be able to authenticate a user
- [x] It should be able to create vegetable gardens
- [x] It should be able to register a plant
- [ ] It should be able to list all plants in third API
- [x] It should be able to schedules reminders for plants
- [ ] It should be able to get plant details
- [ ] It should be able to get user profile 
- [ ] It should be able to get vegetable garden details
- [ ] It should be able to list all plant reminders

## Business Rules

- [x] User can't schedules a duplicate reminder for the same plant
- [x] User can't schedules reminders for plants that's not in their vegetable garden
- [x] User can't register with duplicated email
- [ ] User can't register vegetable gardens or reminders without being authenticated
- [x] User must exists to create vegetable gardens
- [x] It can't register a duplicate plant
- [x] User can't schedules reminders for inexistent plants or gardens